...
func RowToColumns(row *spanner.Row) (Columns, error) {
	columns := Columns{
		Names:           make([]string, row.Size()),
		FormattedValues: make([]string, row.Size()),
	}
	for i := 0; i < row.Size(); i++ {
		var c spanner.GenericColumnValue
		if err := row.Column(i, &c); err != nil {
			return Columns{}, err
		}

		decoded, err := DecodeColumn(c, true)
		if err != nil {
			return Columns{}, err
		}

		columns.Names[i] = row.ColumnName(i)
		columns.FormattedValues[i] = decoded
	}
	return columns, nil
}
...
func DecodeRow(row *spanner.Row, wQuotes bool) ([]string, error) {
	...
	for i := 0; i < row.Size(); i++ {
		...
		decoded, err := DecodeColumn(column, wQuotes)
		if err != nil {
			return nil, err
		}
		columns[i] = decoded
...
func DecodeColumn(column spanner.GenericColumnValue, wQuotes bool) (string, error) {
	...
	case sppb.TypeCode_ARRAY:
		...
		case sppb.TypeCode_STRING:
			...
			for _, v := range vs {
				decoded = append(decoded, withQuotes(nullStringToString(v), wQuotes))
			}
		case sppb.TypeCode_TIMESTAMP:
			...
			for _, v := range vs {
				decoded = append(decoded, withQuotes(nullTimeToString(v), wQuotes))
			}
		case sppb.TypeCode_DATE:
			...
			for _, v := range vs {
				decoded = append(decoded, withQuotes(nullDateToString(v), wQuotes))
			}
		case sppb.TypeCode_STRUCT:
			...
			for _, v := range vs {
				columns, err := DecodeRow(&v.Row, wQuotes)
				if err != nil {
					return "", err
				}
	...
	case sppb.TypeCode_STRING:
		...
		if err := column.Decode(&v); err != nil {
			return withQuotes("", wQuotes), err
		}
		return withQuotes(nullStringToString(v), wQuotes), nil
	case sppb.TypeCode_TIMESTAMP:
		var v spanner.NullTime
		if err := column.Decode(&v); err != nil {
			return withQuotes("", wQuotes), err
		}
		return withQuotes(nullTimeToString(v), wQuotes), nil
	case sppb.TypeCode_DATE:
...
func withQuotes(s string, withQuotes bool) string {
	if withQuotes && s != "NULL" {
		return "'" + s + "'"
	}
	return s
}
